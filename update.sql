...
ALTER TABLE `lamp_nestjs`.`c_menu`
    ADD COLUMN `deleted_at` datetime(0) COMMENT '删除时间' AFTER `readonly_`;
ALTER TABLE `lamp_nestjs`.`c_area`
...
ALTER TABLE `lamp_nestjs`.`c_menu`
    ...
    CHANGE COLUMN `update_time` `updated_at` datetime(0) DEFAULT NULL COMMENT '更新时间' AFTER `updated_by`;
ALTER TABLE `lamp_nestjs`.`c_area`
    CHANGE COLUMN `level` `level_` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT '' COMMENT '行政区级 \n@Echo(api = DICTIONARY_ITEM_CLASS,  dictType = EchoDictType.AREA_LEVEL)' AFTER `latitude`;
ALTER TABLE `lamp_nestjs`.`c_area`
